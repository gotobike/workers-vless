...
// UUID | AUTH | uuid  UUID
// 定制修改于 2025.10.3
// 修复cfip无法使用 ip:port#alias 格式，其中alias无法使用中文及特殊字符，未使用相关逻辑


import { connect } from 'cloudflare:sockets';

let subPath = 'link';     // 订阅路径,不修改将使用uuid作为订阅路径
let password = '112358';  // 主页密码，建议修改或添加PASSWORD环境变量
let serverPool = ['198.12.95.225', '61.222.202.211', '122.117.174.80', 'kr.tp50000.netlib.re', 'ProxyIP.Multacom.CMLiussss.net','23.162.136.169'];  // proxyIP
let yourUUID = 'b284a1ce-d350-437f-8c5c-851757655997'; // UUID

let cfip = [ // cfip
	'skk.moe', 'www.visa.com.tw', 'www.visa.com.hk', 'www.visa.com.sg',
	'cf.090227.xyz','cf.877774.xyz', '172.64.146.41:8443#SG', '104.18.44.163:8443#SG',
    '172.64.229.224:8443#JP', '104.16.84.158:8443#US'
]; 
...
function getVLConfig(yourUUID, url) {
	...
	const configs = addresses.map(addr => {
        let serverAddr = addr;
        let serverPort = '443'; // 默认端口
		let alias = 'iFido'; // 默认别名
        ...
        return `${header}://${yourUUID}@${serverAddr}:${serverPort}?encryption=none&security=tls&sni=${url}&fp=chrome&type=ws&host=${url}&path=${encodedPath}#${alias}`;});
...
<body>
    <div class="login-container">
        ...
        <div class="footer">
            <p>Powered by Cloudflare <a href=" " target="_self" style="color: #007bff; text-decoration: none;">Workers</a></p>
        </div>
    </div>
...
<body>
    ...
    <div class="container">
        <div class="logo">🚀</div>
        <h1 class="title">KiWi</h1>
        <p class="subtitle">基于 Cloudflare 的科学上网服务</p>
        
        <div class="info-card">
            <div class="info-item">
                <span class="label">服务状态</span>
                <span class="value"><span class="status"></span>正在运行</span>
            </div>
            <div class="info-item">
